...
export const CALENDRIER_GROSSESSE: CalendarEvent[] = [
  ...
  },
  {
    ...
    conseils: [
      'Déclaration de grossesse (CERFA)',
  ...
  {
    id: 'declaration_grossesse_cerfa',
    titre: 'Déclaration de grossesse (CERFA)',
    description: 'Déclaration obligatoire avant la fin de la 14e SA',
    saMin: 6,
    saMax: 14,
    type: 'examen',
    priorite: 'obligatoire',
    examens: [
      ...
      'Envoyer à la CPAM et à la CAF',
      'Double exemplaire (rose et bleu)'
    ],
    conseils: [
      'À faire lors de la première consultation',
      'Obligatoire avant 14 SA révolues',
      'Permet le remboursement à 100% des examens',
      'Active les prestations maternité'
    ]
  },
  {
    id: 'biologie_t1',
    ...
    examens: [
      ...
      'NFS (Hb, plaquettes)',
      'Glycémie à jeun',
      'Sérologies: Toxoplasmose, Rubéole, Syphilis, HIV, Hépatite B, Hépatite C (VHC)',
      'ECBU',
      'TSH si facteurs de risque'
    ]
  },
  {
    id: 'recherche_rhesus_foetal',
    titre: 'Recherche de Rhésus fœtal',
    description: 'Détermination du Rhésus fœtal sur sang maternel (si mère Rh-)',
    saMin: 11,
    saMax: 13,
    type: 'biologie',
    priorite: 'recommande',
    examens: [
      ...
      'Évite les injections inutiles de gammaglobulines anti-D'
    ],
    conseils: [
      'Uniquement si la mère est Rhésus négatif',
      'Si le fœtus est Rh+, prophylaxie anti-D nécessaire',
      'Si le fœtus est Rh-, pas de prophylaxie nécessaire',
      'Remboursé par la Sécurité Sociale'
    ]
  },
  {
    id: 'depistage_trisomie21',
    titre: 'Dépistage de la trisomie 21',
    description: 'Dépistage combiné du 1er trimestre (T21, T18, T13)',
    saMin: 11,
    saMax: 13,
    type: 'depistage',
    priorite: 'recommande',
    examens: [
      'Prise de sang (marqueurs sériques: PAPP-A et β-HCG)',
      'Échographie (mesure de la clarté nucale)',
      'Calcul du risque combiné',
      'Si risque élevé: proposition DPNI ou amniocentèse'
    ],
    conseils: [
      'Dépistage proposé, non obligatoire',
      'Information et consentement de la patiente requis',
      'Taux de détection ~90%',
      'DPNI (Test prénatal non invasif) en 2e intention si risque >1/1000'
    ]
  },
  {
    id: 'echo_t1',
...
export function getConsultationRecommendations(currentSA: number): {
  ...
  // Récupérer tous les événements en cours (consultations + examens)
  for (const event of current) {
    ...
    // Ajouter les prescriptions pour les examens/échographies/biologies à prévoir
    if (event.type === 'echographie' || event.type === 'biologie' || event.type === 'examen' || event.type === 'depistage') {
      prescriptionsAPrevoir.push(event.titre)
    }
  }

  // Identifier les examens en retard (passés mais obligatoires)
  for (const event of passed) {
    if ((event.type === 'echographie' || event.type === 'biologie' || event.type === 'examen' || event.type === 'depistage') &&
        (event.priorite === 'obligatoire' || event.priorite === 'recommande')) {
      ...
      // Reporter les examens manqués à la consultation actuelle
      if (event.examens) {
        examensAFaire.push(...event.examens.map(ex => `⚠️ RETARD: ${ex} (${event.titre} ${event.saMin}-${event.saMax} SA)`))
      }
      prescriptionsAPrevoir.push(`⚠️ À RATTRAPER: ${event.titre} (était prévu ${event.saMin}-${event.saMax} SA)`)
    }
  }
